...
class LineOffset {
  ...
  constructor (
    ...
    if (!["center", "left", "right"].includes(alignmentPoint)) {
      throw new Error(`Invalid alignment point ${alignmentPoint}.`);
    }
...
export class BadgeLabelBuilder {
  ...
  static get lineOffsets() {
    return {
      ...
      // maxX of -1 means the label width
      // offsetX of -1 means center it
      "line1": new LineOffset("center", true, -1, 110, -1, 50),
      "line2": new LineOffset("center", true, -1, 53, -1, 130),
      "level": new LineOffset("center", false, 400, 40, -1, 175),
  ...
  static async loadCustomFonts(doc) {
    ...
    let font = new FontFace("twemoji", 'url(fonts/TwitterColorEmoji-SVGinOT.woff2)', {
   ...
   * @returns {LabelEpl} Label with the badge content added.
   */
  addToLabel(label) {
    // Safety buffer around the edges of the printable area.
    // Make sure the width ends up on an byte boundary!
    let widthSafeZone = 20;
    let widthRemainder = (label.labelWidthDots - widthSafeZone) % 8;
    let widthOffset = widthSafeZone + widthRemainder;
    let canvasData = this.renderToImageData(
      label.labelWidthDots - widthOffset,
      label.labelHeightDots - 10);

    return label.setOffset(widthOffset / 2).addImage(canvasData);
  }
  ...
  #addLine(ctx, offsets, maxWidth, text) {
    const maxX = offsets.maxWidth > -1 ? offsets.maxWidth : maxWidth;
    const offsetX = offsets.offsetX > -1 ? offsets.offsetX : (maxWidth / 2);
    const bold = offsets.isBold;

    let size = this.#getMaxFontSizeInPx(ctx, text, maxX, offsets.maxHeight, bold);
    console.log(text, maxWidth, size, offsets);
    ctx.textAlign = offsets.alignmentPoint;
    ctx.font = `${bold ? "bold" : ""} ${size}px ${this.constructor.fontFamily}`;
  ...
  #getMaxFontSizeInPx(ctx, text, maxWidth, maxHeight, isBold) {
    ...
    ctx.font = `${isBold ? "bold" : ""} 1px ${this.constructor.fontFamily}`;
    ...
    // descenders calculated. This means allcaps is bigger for a given hegith limit.
